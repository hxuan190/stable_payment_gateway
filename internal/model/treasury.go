...
type TreasuryWallet struct {
	...
	Blockchain  Chain  `gorm:"type:varchar(20);not null;uniqueIndex:idx_blockchain_address" json:"blockchain"`
...
type TreasuryOperation struct {
	...
	Blockchain       Chain           `gorm:"type:varchar(20);not null;index" json:"blockchain"`
	...
	Signers              map[string]interface{} `gorm:"type:jsonb" json:"signers,omitempty"` // {signer_address, signed_at, signature}
...
func (to *TreasuryOperation) AddSignature(signerAddress, signature string) {
	...
	if to.Signers == nil {
		to.Signers = make(map[string]interface{})
	}
