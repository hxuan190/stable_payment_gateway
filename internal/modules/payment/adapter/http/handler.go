...
// @Success 201 {object} APIResponse{data=CreatePaymentResponse}
...
func (h *PaymentHandler) CreatePayment(c *gin.Context) {
	...
	serviceReq := port.CreatePaymentRequest{
	...
	if req.Chain != "" {
		serviceReq.Chain = domain.Chain(req.Chain)
	}
	...
	if payment.AmountCrypto.GreaterThan(travelRuleThreshold) {
		if req.TravelRule == nil {
			...
			c.JSON(http.StatusBadRequest, ErrorResponse(
				...
				fmt.Sprintf("Travel Rule data is required for transactions exceeding $%.2f USD", travelRuleThreshold.InexactFloat64()),
			))
			return
		}
...
// @Success 200 {object} APIResponse{data=GetPaymentResponse}
// @Failure 400 {object} APIResponse
// @Failure 401 {object} APIResponse
...
func (h *PaymentHandler) GetPayment(c *gin.Context) {
	...
	if paymentID == "" {
		c.JSON(http.StatusBadRequest, ErrorResponse("INVALID_REQUEST", "Payment ID is required"))
		return
	}
	...
	if payment.MerchantID != merchant.ID {
		...
		c.JSON(http.StatusForbidden, ErrorResponse("FORBIDDEN", "Access denied to this payment"))
		return
	}
...
// @Success 200 {object} APIResponse{data=ListPaymentsResponse}
// @Failure 400 {object} APIResponse
// @Failure 401 {object} APIResponse
// @Failure 500 {object} APIResponse
...
func (h *PaymentHandler) ListPayments(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusInternalServerError, ErrorResponse("INTERNAL_ERROR", "Failed to retrieve payments"))
		return
	}

	// Convert to list items
	paymentItems := make([]PaymentListItem, len(payments))
	for i, payment := range payments {
		paymentItems[i] = PaymentToListItem(payment)
	}
...
func (h *PaymentHandler) generateQRCode(payment *domain.Payment) (string, error) {
...
// @Produce json
// @Param id path string true "Payment ID"
// @Success 200 {object} APIResponse{data=PaymentStatusResponse}
// @Failure 404 {object} APIResponse
// @Failure 500 {object} APIResponse
...
func (h *PaymentHandler) GetPublicPaymentStatus(c *gin.Context) {
	...
	response := PaymentToPublicStatusResponse(payment)
...
func (h *PaymentHandler) mapServiceError(err error) (statusCode int, errorCode string, errorMessage string) {
	...
	case errors.Is(err, domain.ErrMerchantNotApproved):
