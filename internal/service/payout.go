...
func (s *PayoutService) RequestPayout(input RequestPayoutInput) (*model.Payout, error) {
	...
	merchant, err := s.merchantRepo.GetByID(input.MerchantID)
	if err != nil {
	...
	if merchant.KYCStatus != model.KYCStatusApproved {
		return nil, ErrPayoutMerchantNotApproved
	}

	// Get merchant balance
	balance, err := s.balanceRepo.GetByMerchantID(input.MerchantID)
	if err != nil {
	...
	totalRequired := input.AmountVND.Add(feeVND)

	// Check if merchant has sufficient withdrawable balance
	if !balance.CanWithdraw(totalRequired) {
		return nil, fmt.Errorf("%w: required %s VND (including %s VND fee), available %s VND",
			ErrPayoutInsufficientBalance,
			totalRequired.String(),
			feeVND.String(),
			balance.GetWithdrawableBalance().String(),
		)
	}
	...
	// Reserve balance (lock it so it can't be used for other payouts)
	if err := s.balanceRepo.ReserveBalanceTx(tx, input.MerchantID, totalRequired); err != nil {
		return nil, fmt.Errorf("failed to reserve balance: %w", err)
	}

	// Record in ledger - reserve merchant balance
	if err := s.ledgerSvc.RecordPayoutRequested(payout.ID, input.MerchantID, totalRequired); err != nil {
		return nil, fmt.Errorf("failed to record payout in ledger: %w", err)
	}
...
func (s *PayoutService) RejectPayout(payoutID, reason string) error {
	...
	// Calculate total reserved amount (amount + fee)
	totalReserved := payout.AmountVND.Add(payout.FeeVND)

	// Release reserved balance
	if err := s.balanceRepo.ReleaseReservedBalanceTx(tx, payout.MerchantID, totalReserved); err != nil {
	...
	// Record in ledger - payout rejected
	if err := s.ledgerSvc.RecordPayoutRejected(payoutID, payout.MerchantID, totalReserved); err != nil {
		return fmt.Errorf("failed to record payout rejection in ledger: %w", err)
	}
...
func (s *PayoutService) CompletePayout(payoutID, bankReferenceNumber, processedBy string) error {
	...
	// Deduct balance from merchant (amount + fee)
	if err := s.balanceRepo.DeductBalanceTx(tx, payout.MerchantID, payout.AmountVND, payout.FeeVND); err != nil {
		return fmt.Errorf("failed to deduct merchant balance: %w", err)
	}

	// Record in ledger - payout completed
	if err := s.ledgerSvc.RecordPayoutCompleted(payoutID, payout.MerchantID, payout.AmountVND, payout.FeeVND); err != nil {
		return fmt.Errorf("failed to record payout in ledger: %w", err)
	}
...
func (s *PayoutService) FailPayout(payoutID, failureReason, processedBy string) error {
	...
	// Record in ledger - payout failed
	if err := s.ledgerSvc.RecordPayoutFailed(payoutID, payout.MerchantID, totalReserved, failureReason); err != nil {
		return fmt.Errorf("failed to record payout failure in ledger: %w", err)
	}
