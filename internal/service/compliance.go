...
func (s *complianceServiceImpl) ValidatePaymentCompliance(
	...
	err := s.CheckMonthlyLimit(ctx, payment.MerchantID, payment.AmountUSD)
	if err != nil {
...
func (s *complianceServiceImpl) StoreTravelRuleData(ctx context.Context, data *model.TravelRuleData) error {
	...
	auditLog := &model.AuditLog{
		...
		ResourceID:   uuid.MustParse(data.PaymentID),
...
func (s *complianceServiceImpl) UpgradeKYCTier(ctx context.Context, merchantID string, tier string) error {
	...
	auditLog := &model.AuditLog{
		...
		ResourceID:   uuid.MustParse(merchantID),
		Metadata: map[string]interface{}{
			"old_tier":          oldTier,
			"new_tier":          tier,
